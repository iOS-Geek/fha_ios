...
@interface ViewController () <CustomToolBarVC_DELEGATE,timerDelegate, UIActionSheetDelegate,UserVideoPanelDELEGATE,InfoViewController_DELEGATE,TableList_DELEGATE,ratingDelegate> {
...
- (void)checkAndGoBack {
    ...
    //if (!isViewInTransmitMode) {
        ...
        LogInVC *vc=(LogInVC*)[self.navigationController.viewControllers objectAtIndex:self.navigationController.viewControllers.count-2];
        [vc.timer invalidate];
        vc.timer=nil;
...
//        [self.navigationController setViewControllers:array animated:YES];
     ...
     [self performSelector:@selector(showRatingView) withObject:self afterDelay:0.0];
...
-(void)showRatingView{
    ...
    feedBackView=(FeedBackView*)[storyboard instantiateViewControllerWithIdentifier:@"FeedBackView"];
        feedBackView.ratingDelegate=self;
        [FHDelegate.window addSubview:feedBackView.view];
...
- (void)setNavigationBarProfileButtonShow:(BOOL)show {
    ...
    timerItem = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 60, 40)];
    timerItem.text=@"00:00";
    timerItem.font=[UIFont FHFontBoldWithSize:16];
    timerItem.textAlignment=NSTextAlignmentRight;
    timerItem.textColor=[UIColor whiteColor];
    NSLog(@"%@ vieew description",[[self.navigationController.viewControllers objectAtIndex:self.navigationController.viewControllers.count-2] description]);
...
//
    ...
    UIBarButtonItem *btnInternetConnection = [[UIBarButtonItem alloc] initWithCustomView:timerItem];
    self.navigationItem.rightBarButtonItems = @[ btnInternetConnection /* fixedSpaceBarButtonItem,*/];
    
}

- (void)initConferenceTextField {
    _txt_conferenceId.text = _txt_conferenceIdText;
    _txtDisplayName.text=_txtDisplayNameText;
...
- (IBAction)okButtonPressed:(int)rating {
    ...
    [dict setValue:_bookingId forKey:@"bookings_id"];
    
    int ratingValue=rating;
    [dict setValue:[NSString stringWithFormat:@"%d",ratingValue] forKey:@"rating_value"];
    [RequestManager getFromServer:@"rate" parameters:dict completionHandler:^(NSMutableDictionary *responseDict) {
