...
- (void) composeView
{
   ...
   [self adjustTextInputHeightForText:@"" animated:NO];
    ...
    lblPlaceholder.text = @"Post Your Answer...";
	...
	sendButton.frame = CGRectMake(size.width - 58.0f, 0.0f, 50.0f, 40.0f);
...
- (void)layoutSubviews
{
    ...
    CGFloat x = 70;
    CGFloat w = self.frame.size.width - attachButton.frame.size.width - emojiButton.frame.size.width - 10 - (sendButton.hidden ? 0 : (sendButton.frame.size.width+3));
    CGFloat d = 0;
...
- (void) adjustTextInputHeightForText:(NSString*)text animated:(BOOL)animated
{
   ...
   int h2 = [text sizeForFont:textView.font constrainedToSize:CGSizeMake(textView.frame.size.width - 16, 100.0f) lineBreakMode:NSLineBreakByWordWrapping].height;
   ...
   [UIView animateWithDuration:(animated ? .1f : 0) animations:^
    {
       ...
       textView.frame = r;
        ...
        [_delegate inputBoxHieght:self.frame.size.height];
    }];
}
